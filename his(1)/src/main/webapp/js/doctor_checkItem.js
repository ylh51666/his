$(function(){
	...
	$.ajax({
		...
		success:function(data){
			for(var i = 0;i<data.length;i++){
				$("#checkitem").append("<option data-price='"+data[i].price+"' data-id='"+data[i].id+"' value='"+data[i].id+"'>"+data[i].itemname+"</option>");
			}
		}
	})
	...
	$(document).on("click","#submit",function(){
		...
		trs.each(function(){ 
			...
			jsonStrat = jsonStrat + "{\"pid\":\""+pid+"\",\"cid\":\""+tds.eq(0).attr("data-cid")+"\",\"amount\":\""+tds.eq(1).children().val()+"\",\"paystatus\":\"1\"},";
		})
		jsonStrat = jsonStrat.substring(0,jsonStrat.length-1)+"]";
		$.ajax({
			type:"post",
			url:"saveCheckItemRecord",
			data:{
				ds:jsonStrat
			},
			success:function(){
